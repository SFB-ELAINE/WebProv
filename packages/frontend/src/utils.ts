...
export const makeConnection = (a: ProvenanceNode, b: ProvenanceNode, opts?: { doConnection?: boolean }): boolean => {
  const { doConnection = true } = opts || {};

  switch (a.type) {
    case 'model-building-activity':
      if (b.type === 'wet-lab-data') {
        if (doConnection) {
          a.wetLabsUsedForValidation.push(b);
        }
        return true;
      }

      if (b.type === 'simulation-data') {
        if (doConnection) {
          a.simulationsUsedForValidation.push(b);
        }
        return true;
      }
    ...
    case 'model-exploration-activity':
      ...
      return false;
    case 'model':
      if (b.type === 'model-building-activity') {
        if (doConnection) {
          a.wasGeneratedBy = b;
        }
        return true;
    ...
    case 'simulation-data':
      if (b.type === 'model-building-activity') {
        if (doConnection) {
          a.wasGeneratedByModelBuildingActivity = b;
          return true;
        }
      }

      if (b.type === 'model-exploration-activity') {
        if (doConnection) {
          a.wasGeneratedByModelExplorationActivity = b;
        }
        return true;
      }
...
export const isValidConnection = (a: ProvenanceNode, b: ProvenanceNode): boolean => {
  return makeConnection(a, b, { doConnection: false });
};
