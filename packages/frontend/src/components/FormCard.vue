    ...
    >
      ...
      <div v-else-if="field.multiple">
        <div v-for="(value, j) in node[field.name]" :key="`${field.name}-${j}`" style="display: flex">
          <b-field>
            <b-input placeholder="Key" :value="value[0]" @input="update(field.name, value, 0, $event)"></b-input>
          </b-field>
          <div style="flex: 0 0 10px"></div>
          <b-field>
            <b-input placeholder="Value" :value="value[1]" @input="update(field.name, value, 1, $event)"></b-input>
            <p class="control">
...
export default class FormCard<T extends { [k: string]: Values | undefined }> extends Vue {
  ...
  public update(key: string, value: [string, string], i: 0 | 1, newValue: string) {
