...
The approach is implemented in \href{https://github.com/SFB-ELAINE/WebProv}{\webprov{}}, which allows one to insert and query provenance information.
...
By this, it helps to integrate a new simulation model within a family of existing ones.
...
Provenance information is useful for understanding the creation of a simulation model, because it does not only contain information about the entities and activities that have led to a simulation model but also the relations of these, which can be visualized.
...
The resulting provenance graph visualizes the story line within simulation studies and demonstrates the creation and calibration of simulation models, the successive attempts of validation and extension, and shows, beyond an individual simulation study, how the \wnt{} models are related.
...
For example, querying the BioModels database for biochemical and cellular simulation models that contain proteins such as \wnt{}, Janus kinase (Jak), or mitogen-activated protein kinase (MAPK), which are associated with corresponding signaling pathways, returns for \wnt{} a set of 22 simulation models, for Jak a set of 12 simulation models and for MAPK a set of 139 simulation models (as of January 2021).
...
In general, simulation models are the outcome of extensive as well as interactive model and data generation activities that include, in addition to executing various simulation experiments and successive model refinements, the adaptation of already existing models, for example, by composition or extension~\cite{Peng2017, Peng2016, Cvijovic2014}.
...
We will present and use our web-based tool \webprov{} to store, present and query provenance information of these simulation studies.
...
While reading a study, a first sketch of a possible provenance graph was being made.
Afterwards, a revision of all markings helped to finalize the graph and to remove duplicated entities.
...
The tool can be locally installed for testing and replicability purposes.
Details about its installation, as well as the code are found on \href{https://github.com/SFB-ELAINE/WebProv}{GitHub}.
An informative video showing the usage of \webprov{} is found on \href{https://youtu.be/UzwHtptkYOU}{YouTube}.
...
For example, the entity \SM{} can only be created by either the activity \BSM{} or \CSM{} (see the \nameref{sec:ResultsAndDiscussion} section and Table~\ref{tab:entitiesandactivities} for all details).
These classifications can be easily changed and extended if needed.
...
The specialization of PROV-DM, which has been introduced by Ruscheinski et al. (2018)~\cite{Ruscheinski2018}, is being revised and refined.
...
In the case of simulation studies, it typically forms the starting point in the modeling and simulation life cycle~\cite{Balci2012, Robinson2008} and is central to interpret its products such as simulation data or simulation model.

As for our provenance example shown in Fig~\ref{fig:ProvLee}, \lee{}~\cite{Lee2003} questioned in \texttt{RQ1} the necessity of \enquote{the two scaffold proteins, APC and Axin} (for \wnt{} signaling) and whether \enquote{their roles differ}.
...
\noindent Requirements (for a simulation model) define properties the results of a simulation need to show.
...
These real-world measurements determine the species of interest which should be part of the entity \R{}.
...
Formal approaches to describe a system through qualitative models (e.g., Boolean models~\cite{Wang2012} or Petri nets~\cite{Chaouiya2007}) come with their own means of analysis and are assigned to the entity \SM{}, as they are executable models.
...
To further structure the set of simulation experiments applied, we distinguish simulation experiments whether they are used for optimization, sensitivity analysis, perturbation, parameter scan, steady-state analysis, or time course analysis.
...
We have identified optimization experiments to be all experiments where an implicit or explicit objective function is being used.
...
In a sensitivity analysis, more than one parameter value is being changed and some kind of sensitivity coefficient is being calculated.
As perturbations, we have interpreted experiments where the value of one (or more) parameter is being changed to another (just one) value, for example, to mimic a knock-out experiment.
...
Note that we have not included an entity \WE{}.
...
The simulation data are generated by simulation experiments, therefore a link needs to be established.
In case of a simulation experiment that serves the role of validation, information about the success of a validation facilitates to interpret the simulation model and further activities based on the simulation model.
...
The type of the wet-lab experiment (\textit{in vitro} or \textit{in vivo}) as well as the used organism and organ/ tissue/ cell line are recorded.
...
The data of this wet-lab experiment is not shown in the publication.
...
\noindent The \BSM{} activity, also called model derivation~\cite{Vera2021}, can use all entities types, as any entity described above can contribute to the model building process, but it needs to have at least a link to a \RQ{} or \SM{}.
The only result of the building simulation activity is an entity \SM{}.
...
The \BSM{} activity \texttt{BSM2} is extending the simulation model \texttt{SM2}.
...
In the case of the activity~\texttt{CSM1} of \lee{}~\cite{Lee2003}, several wet-lab data from their own experiments (\texttt{WD1}--\texttt{WD5}) as well as from Salic et al. (2000)~\cite{Salic2000} (\texttt{WD1}--\texttt{WD3}) are being used during the calibration of the model \texttt{SM1} which produces a \SE{}~\texttt{SE1}, the corresponding \SD{}~\texttt{SD1} as well as the calibrated \SM{}~\texttt{SM2}.
...
We have not yet considered the type \texttt{agent} of PROV-DM in our approach because this information appeared less relevant in the analyzed simulation studies.
In the future, it could also be part of the provenance information to name the agent an entity is attributed to, the agent an activity is associated with, or to name the agent another agent acted on behave of~\cite{Belhajjame2013}.
This would particularly be of relevance if models are not only validated but also accredited, which typically involves a different group of people than those who have developed the simulation model~\cite{Balci1997}.
...
Or a validation activity that failed and that is followed by a \BSM{} activity obviously holds some information for the latter.
...
Based on the identified and also defined entities and activities from above, we have recorded the provenance information of the 19 studies shown in bold in Table~\ref{tab:wntmodels} as well as the provenance information of entities from other publications that were used by the 19 studies.
...
Screenshots and presentations with the provenance information may also be found on \href{https://github.com/SFB-ELAINE/SI_Provenance_Wnt_Family}{GitHub}.
...
Also, the nonlinearity of the text---it is not a lab protocol after all---makes it hard to identify the relations of the entities and activities as well as the order of their execution.
...
Therefore, provenance information should be directly collected during the simulation study and ideally without an intervention of the modeler.

% Observations of entities of the 19 studies

The \RQ{} was usually found repetitively within the abstract and throughout the introduction and discussion sections.
...
On the other hand, we might have easily missed an assumption and also interpreted statements as assumptions that were not meant as such by the authors.
As a consequence, the assumptions might look different if the authors had defined them themselves.

In order to further analyze the assumptions, we have categorized these using the Systems Biology Ontology (SBO)~\cite{Courtot2011}.
...
The reason for this might partly be because support for thorough experimentation with rule-based models including calibration and validation has only become available during the last decade~\cite{Warnke2017, Thomas2015, Sorokin2019}.

We have categorized all 145 \SEs{} that we found regarding which experiment type they serve (see UML class diagram shown in Fig~\ref{fig:uml}).
The result of the categorization are shown in Fig~\ref{fig:SEcategories} and the details in~\nameref{S1_TableExperiments}.
...
Sometimes, simulation or wet-lab experiments have been conducted, however, the corresponding \SD{} or \WD{} is not shown, but briefly described, and are, thus, without references in the provenance graphs.
...
When looking at \WD{}, six out of 19 publications recorded own wet-lab data \cite{Lee2003, Sick2006, Kim2007, Kogan2012, Haack2015, Staehlke2020}.
...
The latter could only be done because the authors relied on other \SMs{} and the respective parameter and initial values.
Interestingly, the wet-lab data obtained either by the 19 \wnt{} signaling studies or by other studies which where used in the 19 studies stemmed from different organisms and cell lines.
Besides human and murine cell lines (each 11 studies), xenopus, rat, hamster, and kangaroo rat were used as a model organisms.
The experiments included, among others, (tumor) cell lines from the kidney (BHK, HEK293, PtK2), bone (MG-63), cervix (HeLa), brain (neural progenitor cells), and of fibroblasts (L cells, NIH/3T3).
...
The simulations models developed in \cite{Kruger2004, Kim2007, vanLeeuwen2007, Wawra2007, Goldbeter2008, vanLeeuwen2009, Mirams2010} have neither been calibrated nor validated explicitly, because they have used parameter values from other studies.
Sick et al. (2006)~\cite{Sick2006} have used arbitrary parameter values in their simulation models.
...
For example, in~\cite{Mazemondet2012}, the core model of the \wnt{}/\bcat{} signaling pathway has been calibrated with wet-lab data from \lee{}~\cite{Lee2003}, but this calibrated simulation model was not used any longer and instead, a new calibration with wet-lab data from another study took place.
...
This may be valid as the \wnt{}/\bcat{} signaling pathway is evolutionarily conserved~\cite{Steinhart2018}, which means that data can be shared, even though care must always be taken when using, for instance, parameter values determined with one cell line in a study that uses another cell line.
...
A second cluster forms around the simulation studies \cite{Mazemondet2012, Kogan2012, Haack2015, Haack2020, Staehlke2020} and either includes the same wet-lab data from~\cite{Bafico2001, Hannoush2008} or the Cell cycle~\cite{Mazemondet2012} or ROS~\cite{Haack2015, Staehlke2020}.
A third cluster includes the pathways of Notch~\cite{Rodriguez2007, Wang2013} or Notch + MAPK/ERK \cite{Goldbeter2008}. Even though the algorithms locate \cite{Kruger2004} in the same cluster, it is content-wise rather part of the central cluster.
A forth cluster forms around studies including MAPK/ERK~\cite{Kim2007} or MAPK/ERK + PI3K/Akt~\cite{Padala2017}.
...
When comparing Fig~\ref{fig:WntProvNeato} and \nameref{S1_FigClusters}, we find that the wet-lab data of just three studies, namely from \cite{Bafico2001, Hannoush2008, Dequeant2006} have been reused by simulation studies.
On the one hand, this is surprising as wet-lab data could be reused for parameter estimation or model validation.
...
We are also distinguishing between \BSM{}, \CSM{}, \VSM{}, and \ASM{} activities and connect the entities and activities by using the relations \textit{wasGeneratedBy} and \textit{used}.
In our definitions of the entities and activities, we have aimed at the minimal level of detail, or granularity, of the provenance graph to understand the course of a simulation study.
We have also kept the necessary metadata of the entities and activities minimal to convey both the main idea of the simulation study and the content of each entity and activity.
...
We were able to explicitly show that most studies are connected to one or more other \wnt{} simulation studies, using (parts of) their simulation models, but also various data from wet-lab studies.
Our result marks the outstanding role of the \wnt{} simulation model by \lee{}~\cite{Lee2003} as the origin for most other models in our survey.
...
We think that \webprov{}, or a similar tool, would be a valuable extension of model repositories such as BioModels, as one could see where a simulation model comes from, whether there are other models connected to it, and in which way they are connected.
